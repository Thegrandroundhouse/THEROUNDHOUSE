        ...
        {items.map((item, i) => (
          <div key={item.label} className="flex items-center gap-12 md:gap-20">
            <div className="text-center">
              <div className="font-serif text-5xl font-semibold md:text-6xl bg-gradient-to-b from-gold-light to-gold bg-clip-text text-transparent">
                {item.value}
              </div>
            ...
            {i < items.length - 1 && (
              <div className="hidden h-16 w-px shrink-0 bg-gradient-to-b from-transparent via-gold/30 to-transparent md:block" aria-hidden />
            )}
          </div>
        ))}
      </div>
