...
const variants: Record<
  Variant,
  { bg: string; label: string; title: string; body: string; button: string; buttonHover: string }
> = {
  cream: {
    bg: "bg-[#f7f4ed]",
    label: "text-[#a4845a]",
    ...
    button: "bg-[#a4845a] text-white hover:bg-[#8b6f4a]",
    buttonHover: "",
  },
  ivory: {
    bg: "bg-ivory",
    ...
    button: "border-2 border-gold text-gold hover:bg-gold hover:text-ivory",
    buttonHover: "",
  },
  gold: {
    bg: "bg-[#f5efe6]",
    ...
    button: "bg-charcoal text-ivory hover:bg-charcoal-soft",
    buttonHover: "",
  },
  dark: {
    bg: "bg-ink",
    ...
    title: "text-ivory",
    body: "text-ivory/80",
    button: "bg-gold-light text-ink hover:bg-gold",
    buttonHover: "",
  },
  warm: {
    bg: "bg-[#faf6f0]",
    ...
    title: "text-charcoal",
    body: "text-charcoal/80",
    button: "bg-[#8b6914] text-white hover:bg-[#6d5210]",
    buttonHover: "",
  },
};
...
type Overlay = keyof typeof overlayStyles;
...
export function FeatureBlock({
  ...
  const textBlock = (
    <div className={`flex flex-col justify-center px-8 py-16 md:px-12 md:py-20 lg:px-16 ${v.bg}`}>
      <p className={`text-xs font-medium uppercase tracking-[0.3em] ${v.label}`}>
        {label}
      </p>
      <h2 className={`mt-3 font-serif text-3xl font-semibold tracking-tight md:text-4xl lg:text-5xl ${v.title}`}>
        {title}
      </h2>
      {paragraphs.map((p, i) => (
        <p key={i} className={`max-w-lg text-base leading-relaxed ${i === 0 ? "mt-6" : "mt-4"} ${v.body}`}>
          {p}
        </p>
      ))}
      <Link
        href={buttonHref}
        className={`mt-10 inline-flex w-fit items-center gap-2 rounded-md px-8 py-3.5 text-sm font-medium uppercase tracking-[0.2em] transition ${v.button}`}
      >
        {buttonText}
